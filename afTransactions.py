...
def afCheck(address):
    ...
    for transaction in transactions['result']:
        if transaction['methodId'] == pigcreditingMethodId and transaction['isError'] == '0':
            pigOut[f"Date - {transaction['timeStamp']}"] = f"Hash - https://bscscan.com/tx/{transaction['hash']}"

        if transaction['methodId'] == pigLpcreditingMethodId and transaction['isError'] == '0':
            print(transaction['hash'])
            pigLpOut[f"Date - {transaction['timeStamp']}"] = f"Hash - https://bscscan.com/tx/{transaction['hash']}"

        if transaction['methodId'] == dogCreditingMethodId and transaction['isError'] == '0':
            dogOut[f"Date - {transaction['timeStamp']}"] = f"Hash - https://bscscan.com/tx/{transaction['hash']}"

        if transaction['methodId'] == dogBusdCreditingMethodId and transaction['isError'] == '0':
            dogBusdOut[f"Date - {transaction['timeStamp']}"] = f"Hash - https://bscscan.com/tx/{transaction['hash']}"

        if transaction['methodId'] == dogWbnbCreditingMethodId and transaction['isError'] == '0':
            dogWbnbOut[f"Date - {transaction['timeStamp']}"] = f"Hash - https://bscscan.com/tx/{transaction['hash']}"

        if transaction['methodId'] == afpClaimMethodId and transaction['isError'] == '0':
            afpIn[f"Date - {transaction['timeStamp']}"] = f"Hash - https://bscscan.com/tx/{transaction['hash']}"

        if transaction['functionName'] == afdClaimFunctionName and transaction['isError'] == '0':
            afdIn[f"Date - {transaction['timeStamp']}"] = f"Hash - https://bscscan.com/tx/{transaction['hash']}"

        if transaction['methodId'] == afdLpClaimMethodId and transaction['isError'] == '0':
            afdLpIn[f"Date - {transaction['timeStamp']}"] = f"Hash - https://bscscan.com/tx/{transaction['hash']}"
